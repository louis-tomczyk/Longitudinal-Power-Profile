...
%
%		- norm: normalization factor. With a signal pre-emphasis, the
%           signal may be normalized before the nonlinear pre-emphasis 
%           function to ensure real values after pre-emphasis (e.g.,
%           pre-emphasis with asin function). The value can be passed here 
%           to undo the normalization, thus recovering the power of the 
%           original signal before pre-emphasis. See also the emph field in 
%           DIGITALMOD.
...
if nargin > 2
    errmsg=checkfields(options,{'Vpi','exratio','biasu','biasl','bias',...
        'mode','nch','norm'});
    if ~isempty(errmsg), error('optilux:mzmodulator',errmsg);end
